...
function BubbleSet() {
  ...
  var intersectionPlineA = new Line(0.0, 0.0, 0.0, 0.0);
  Intersection.fractionToLineCenter = function(bounds, line) {
    var minDistance = Number.POSITIVE_INFINITY;
    var countIntersections = 0;

    function testLine(xa, ya, xb, yb) {
      intersectionPlineA.x1(+xa);
      intersectionPlineA.y1(+ya);
      intersectionPlineA.x2(+xb);
      intersectionPlineA.y2(+yb);
      var testDistance = +Intersection.fractionAlongLineA(line, intersectionPlineA);
  ...
  Intersection.fractionToLineEnd = function(bounds, line) {
    ...
    function testLine(xa, ya, xb, yb) {
      intersectionPlineB.x1(+xa);
      intersectionPlineB.y1(+ya);
      intersectionPlineB.x2(+xb);
      intersectionPlineB.y2(+yb);
      var testDistance = +Intersection.fractionAlongLineA(line, intersectionPlineB);
      if((testDistance >= 0) && (testDistance <= 1)) {
        countIntersections += 1;
        if(testDistance < minDistance) {
          minDistance = testDistance;
  ...
  Intersection.testIntersection = function(line, bounds, intersections) {
    ...
    function fillIntersection(ix, xa, ya, xb, yb) {
      intersectionPlineC.x1(+xa);
      intersectionPlineC.y1(+ya);
